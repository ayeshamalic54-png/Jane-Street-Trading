...
def update_dashboard_state(acc_info, daily_loss_p, z_score, beta, alpha, spread, obi_a, obi_b, trades_today, is_halted, active_zones=None):
    ...
    zone_list = []
    if active_zones:
        for low, high in active_zones.get('bullish_ob', []): zone_list.append({"type": "OB", "label": "🟢 OB", "range": f"{low:.5f}-{high:.5f}"})
        for low, high in active_zones.get('bullish_fvg', []): zone_list.append({"type": "FVG", "label": "🟢 FVG", "range": f"{low:.5f}-{high:.5f}"})
        for low, high in active_zones.get('bearish_ob', []): zone_list.append({"type": "OB", "label": "🔴 OB", "range": f"{low:.5f}-{high:.5f}"})
        for low, high in active_zones.get('bearish_fvg', []): zone_list.append({"type": "FVG", "label": "🔴 FVG", "range": f"{low:.5f}-{high:.5f}"})
