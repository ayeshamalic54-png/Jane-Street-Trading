...
def main():
    ...
    while True:
        try:
                ...
                if best_sig is not None:
                    ...
                    if is_long:
                        ...
                        if best_cat_a == "crypto":
                            ...
                            # SINGLE-ASSET DIRECT ORDER EXECUTION FOR SELL (1 Ticket, Zero Hedge Drag)
                            trade_type = mt5.ORDER_TYPE_SELL
                            entry_p = best_sig["tick_a"].bid
                            sl_val = sl_a
                            tp_val = tp2_val
                            ...
                            if res_order and is_retcode_success(res_order.retcode):
                                ...
                                logger.info(f"🟢 [SINGLE DIRECT ORDER FIRED] Symbol: {S_A_resolved} | Action: SELL | Lots: {actual_lots_a:.2f} | Price: {res_order.price:.5f} | SL: {sl_val:.5f} | TP: {tp_val:.5f}")
            ...
            logger.info(
                f"📊 [LIVE SCAN DETAIL] Focus: Single-Asset List ({S_A}) | Live Z: {active_pair_z_score:.3f} (Entry: ±{Z_ENTRY_THRESHOLD:.2f}) | Single-Asset Engine: ACTIVE 🟢 "
