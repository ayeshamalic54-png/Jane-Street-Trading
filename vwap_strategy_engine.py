...
def evaluate_single_asset_vwap_signal(df: pd.DataFrame, z_threshold: float = 2.0, category: str = "forex"):
    """
    Single-Asset VWAP Z-Score Reversal Execution Logic:
    - BUY: Price > 200 EMA (Bullish Trend), Z-Score <= -2.0 then curls UP across -2.0.
      TP = VWAP Line (Z = 0.0), SL = Nearest M15 Swing Low - Buffer.
    - SELL: Price < 200 EMA (Bearish Trend), Z-Score >= +2.0 then curls DOWN across +2.0.
