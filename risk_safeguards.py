...
# Maximum daily drawdown allowed in USD before halting trading (Hard SL cap)
MAX_DAILY_LOSS_USD = 50.0
# Maximum daily drawdown allowed in percentage (fallback)
MAX_DAILY_LOSS_PERCENT = 4.2
# Maximum number of trades allowed per day
MAX_DAILY_TRADES = 3
# Fixed risk per trade in USD (SL per trade = $45 USD)
FIXED_RISK_PER_TRADE_USD = 45.0
# Risk percentage per trade (fallback 1.0%)
RISK_PERCENT = 1.0
...
def check_drawdown_limit(acc_info):
    ...
    if daily_loss >= MAX_DAILY_LOSS_USD or daily_loss_percent >= MAX_DAILY_LOSS_PERCENT:
        logger.critical(f"HARD DAILY LIMIT BREACHED: Daily Loss is ${daily_loss:.2f} USD (Hard Limit: ${MAX_DAILY_LOSS_USD} USD / {MAX_DAILY_LOSS_PERCENT}%)")
...
def calculate_lots(symbol, sl_distance_price, acc_info):
    ...
    # Per trade fixed dollar risk set to $45 USD per trade
    risk_amount = FIXED_RISK_PER_TRADE_USD if FIXED_RISK_PER_TRADE_USD > 0 else equity * (RISK_PERCENT / 100.0)
