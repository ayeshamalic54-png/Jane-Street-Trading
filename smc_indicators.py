...
def detect_smc_zones(df):
    ...
    # Raw detected structures with their creation index
    raw_bullish_obs = []       # [low, high, index]
    raw_bearish_obs = []       # [low, high, index]
    raw_bullish_fvgs = []      # [low, high, index] (Gap between i-2 high and i low)
    raw_bearish_fvgs = []      # [low, high, index] (Gap between i low and i-2 high)
    
    bullish_breakers = []      # [low, high, index] (broken bearish OBs)
    bearish_breakers = []      # [low, high, index] (broken bullish OBs)
    bullish_ifvgs = []         # [low, high, index] (broken bearish FVGs)
    bearish_ifvgs = []         # [low, high, index] (broken bullish FVGs)
    ...
    for i in range(2, n):
        ...
        # A strong up move is defined here when candle i closes above candle i-1 high
    ...
    for ob in raw_bullish_obs:
        ...
        for j in range(ob_idx + 2, n):
            # Broken/Mitigated if close is below OB low
            if closes[j] < ob_low:
                mitigated = True
                # Becomes a Bearish Breaker at index j
                bearish_breakers.append([ob_low, ob_high, j])
                break
            # Mitigated (touched) if low drops below OB low
            elif lows[j] <= ob_low:
                mitigated = True
                break
        if not mitigated:
    ...
    for ob in raw_bearish_obs:
        ...
        mitigated = False
        for j in range(ob_idx + 2, n):
            # Broken/Mitigated if close is above OB high
            if closes[j] > ob_high:
                mitigated = True
                # Becomes a Bullish Breaker at index j
                bullish_breakers.append([ob_low, ob_high, j])
                break
            # Mitigated (touched) if high rises above OB high
    ...
    for fvg in raw_bullish_fvgs:
        ...
        for j in range(fvg_idx + 1, n):
            # Broken (closes below FVG low) -> Bearish iFVG
            if closes[j] < fvg_low:
                mitigated = True
                bearish_ifvgs.append([fvg_low, fvg_high, j])
                break
            # Mitigated if price enters/touches it
            elif lows[j] <= fvg_low:
                mitigated = True
                break
        if not mitigated:
    ...
    for fvg in raw_bearish_fvgs:
        ...
        mitigated = False
        for j in range(fvg_idx + 1, n):
            # Broken (closes above FVG high) -> Bullish iFVG
